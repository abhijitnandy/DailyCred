...
var DAILYCRED_CLIENT_ID = <YOUR CLIENT ID>;
var DAILYCRED_SECRET = <DAILYCRED SECRET>;
...
for (i in providers){
  ...
  app.get("/auth/"+provider, passport.authenticate('dailycred', {identity_provider: provider}))
}
...
app.get('/auth/dc/callback', passport.authenticate('dailycred', { failureRedirect: '/login' }),
function(req, res) {
  res.redirect('/');
});
